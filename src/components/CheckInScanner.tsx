...
interface CheckInScannerProps {
  onCheckIn?: () => void;
  /** Minimal layout: no header/cards, just scanner + button (e.g. for /scanner page) */
  standalone?: boolean;
}
...
export function CheckInScanner({ onCheckIn, standalone = false }: CheckInScannerProps) {
    ...
    >
      {!scanning && (
        <div className="text-center text-slate-500">
          ...
          <p>{standalone ? 'Tap "Start Scanner" to activate camera' : 'Click "Start Scanning" to activate camera'}</p>
        </div>
      )}
    </div>
  );
  ...
  const buttons = (
    <div className="flex gap-2">
      {!scanning ? (
        <Button onClick={startScanning} className="flex-1" size={standalone ? 'lg' : 'default'}>
          <Camera className="h-4 w-4 mr-2" />
          {standalone ? 'Start Scanner' : 'Start Scanning'}
        </Button>
      ) : (
        <Button onClick={stopScanning} variant="destructive" className="flex-1" size={standalone ? 'lg' : 'default'}>
          <X className="h-4 w-4 mr-2" />
          Stop Scanning
        </Button>
      )}
      {!standalone && (
        <Button onClick={() => setScanResult(null)} variant="outline" disabled={scanning}>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Results from the latest QR code scan</CardDescription>
        </CardHeader>
        <CardContent>
          {scanResult ? (
            <div
              ...
              {scanResult.attendee && (
                <div className="mt-4 pt-4 border-t border-current border-opacity-20">
                  ...
                  <div className="text-sm space-y-1">
                    <p>
                      <strong>Name:</strong> {scanResult.attendee.firstName} {scanResult.attendee.lastName}
                    </p>
                    <p>
